...
const LAST_WORD_REGEX = /[\p{L}\p{M}']+(?=\W*$)/u;
...
export default class AutoCorrectPlugin extends Plugin {
	...
	private handleEditorChange(editor: Editor) {
		...
		// 1) Checkbox items ----------------------------------------------
		if (this.settings.capitalizeCheckboxItem) {
			const trimmedIndent = fullLine.replace(/^[\t ]+/, "");
			if (CHECKBOX_REGEX.test(trimmedIndent)) {
				this.correctCheckboxItem(editor, fullLine, lineNo);
				return; // Exit early to avoid processing as regular list
			}
		}
	...
	private correctCheckboxItem(editor: Editor, line: string, lineNo: number) {
		const doc = editor.getDoc();
	...
	private isInProtectedBlock(
		...
		const doc = editor.getDoc();
		const currentLine =
			lineNo !== undefined ? lineNo : doc.getCursor().line;
...
class AutoCorrectSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Capitalize checkbox items")
			.addToggle((toggle) =>
				toggle
